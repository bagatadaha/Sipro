...
async def pdf(quotation_id: str,
    ...
    costs = q.get("costs") or {}
    if costs.get("components"):
        lines.append(f"Biaya all-in ({costs.get('scheme_name') or 'skema'}) :")
        for c in costs["components"]:
            dev = c.get("treatment") == "developer_borne"
            lines.append(f"  {c.get('name') or c.get('code')} : {rp(c.get('amount'))}"
                         + (" (ditanggung developer)" if dev else " (ditagih ke pembeli)"))
        lines.append(f"Total biaya ditagih ke pembeli : {rp(costs.get('buyer_total'))}")
    if q.get("booking_fee") is not None:
        lines.append(f"Booking fee / tanda jadi : {rp(q.get('booking_fee'))}")
    if q.get("buyer_total") is not None:
        lines.append(f"Total dibayar pembeli : {rp(q.get('buyer_total'))}")
    kpr = q.get("kpr") or {}
